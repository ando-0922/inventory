	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			case "3": // クイック入荷 // 入力 
				...
				int warehouseId = disp.entIntNotNull("倉庫 (ID) (必須): _");
				Warehouse warehouse = warehousesSvc.warehouse(warehouseId);
				String jan = oDisp.entJanNotNull("JAN (必須): _");
				Product product = productsSvc.jan(jan);
				int qty = disp.entIntNotNull("数量 (整数>0) (必須): _"); // 確認表示 
				disp.dispMsg("##受入確認##\n" + "仕入先: " + supplier.getName() + "\n倉庫: " + warehouse.getName() + "\n商品: "
						+ jan + " " + product.getName() + "\n数量: " + qty);
				if (!disp.confirm("この内容で実行しますか？ (Y/N): _")) {
					disp.dispMsg("やり直してください。");
					break;
				} // サービス呼び出し 
				int purchaseId = purchasesSvc.quickReceive(product, supplier, warehouse, qty);
				disp.dispMsg("OK: 受入を登録しました (purchase_id=" + purchaseId + ", 新在庫="
						+ stocksSvc.getQty(product, warehouse) + ")");
				break;
			case "4": // クイック販売 
				int slWarehouseId = disp.entIntNotNull("倉庫 (ID) (必須): _");
				Warehouse slWarehouse = warehousesSvc.warehouse(slWarehouseId);
				String slJan = oDisp.entJanNotNull("JAN (必須): _");
				Product slProduct = productsSvc.jan(slJan);
				int slQty = disp.entIntNotNull("数量 (整数>0) (必須): _");
				int slPrice = disp.entInt("売価 [既定=" + slProduct.getStdPrice() + "]: _");
				if (slPrice <= 0)
					slPrice = slProduct.getStdPrice(); // 確認表示 
				disp.dispMsg("##出庫確認##\n" + "倉庫: " + slWarehouse.getName() + "\n商品: " + slJan + " "
						+ slProduct.getName() + "\n数量: " + slQty + " / 単価: " + slPrice + "\n金額: " + (slQty * slPrice));
				if (!disp.confirm("この内容で実行しますか？ (Y/N): _")) {
					disp.dispMsg("やり直してください。");
					break;
				} // サービス呼び出し 
				int saleId = salesSvc.quickSale(slProduct, slWarehouse, slQty, slPrice);
				disp.dispMsg(
						"OK: 出庫しました (sale_id=" + saleId + ", 新在庫=" + stocksSvc.getQty(slProduct, slWarehouse) + ")");
				break;
			//			case "5":
